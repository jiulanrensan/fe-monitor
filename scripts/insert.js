...
function generateTestData(count = 1) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // log_time 比 report_time 早 1-10 秒的随机值
        const timeDiff = Math.floor(Math.random() * 10000) + 1000; // 1-10秒，转换为毫秒
...
const tableName = 'fre_monitor_db.api__duration';
...
const insertSQL = `INSERT INTO ${tableName} (aid, sid, uid, log_time, report_time, retry_times, url, method, status_code, duration, queue_time, queue_start, queue_end, req_page, res_page, network, model) VALUES ${sqlValues}`;
