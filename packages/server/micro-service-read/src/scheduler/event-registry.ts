...
export class EventRegistry {
  ...
  registerEvent(event: MonitorEvent): void {
    if (this.events.has(event.name)) {
      this.logger.warn(`Event ${event.name} already registered, overwriting...`)
    }
  ...
  async executeAllEvents(): Promise<void> {
    ...
    const executionPromises = Array.from(this.events.values()).map(async (event) => {
      try {
        ...
        this.logger.error(`Event ${event.name} execution failed: ${error.message}`, error.stack)
  ...
  async executeEvent(eventName: string): Promise<any> {
    ...
    try {
      ...
      this.logger.error(`Event ${eventName} execution failed: ${error.message}`, error.stack)
      throw error
    }
  }
}
